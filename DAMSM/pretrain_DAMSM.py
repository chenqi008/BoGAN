...
from model_DAMSM import RNN_ENCODER, CNN_ENCODER
...
def parse_args():
    ...
    parser.add_argument('--input_channels', type=int, default=3, help='the number of channel of input images (1 or 3)')
    ...
    parser.add_argument('--OnlyTest', type=str2bool, default=True,
...
def train(dataloader, cnn_model, rnn_model, batch_size,
          labels, optimizer, epoch, ixtoword, image_dir, input_channels):
    cnn_model.train()
    rnn_model.train()
    s_total_loss0 = 0
    s_total_loss1 = 0
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        nef, att_len, att_sze = words_features.size(1), words_features.size(2), words_features.size(3)
...
def test(dataloader, cnn_model, rnn_model, batch_size,
          labels, ixtoword, image_dir, input_channels):
    cnn_model.eval()
    rnn_model.eval()
    # s_total_loss0 = 0
    # s_total_loss1 = 0
...
def build_models():
    ...
    image_encoder = CNN_ENCODER(cfg.TEXT.EMBEDDING_DIM, args.input_channels)
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(args.output_dir, '%s_%s_%s' % (cfg.DATASET_NAME, cfg.CONFIG_NAME, timestamp))
    ...
    if args.OnlyTest:
        ...
        if args.checkpoint_text_encoder != '':
            text_encoder.load_state_dict(torch.load(args.checkpoint_text_encoder, map_location="cuda:%s"%(cfg.GPU_ID)))
        if args.checkpoint_image_encoder != '':
            image_encoder.load_state_dict(torch.load(args.checkpoint_image_encoder, map_location="cuda:%s"%(cfg.GPU_ID)))

        step = test(dataloader, image_encoder, text_encoder,
                              batch_size, labels,
                              dataset.ixtoword, image_dir, args.input_channels)
        print('Finish')
    else:
        try:
